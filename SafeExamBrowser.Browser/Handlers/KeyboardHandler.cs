	...
	internal class KeyboardHandler : IKeyboardHandler
	{
		...
		public bool OnKeyEvent(IWebBrowser browserControl, IBrowser browser, KeyType type, int keyCode, int nativeKeyCode, CefEventFlags modifiers, bool isSystemKey)
		{
			...
			if (type == KeyType.KeyUp)
			{
				...
				if (ctrl && keyCode == (int) Keys.T)
				{
					NewTabRequested?.Invoke();
				}

				if (ctrl && keyCode == (int) Keys.W)
				{
					CloseTabRequested?.Invoke();
				}

				if (ctrl && keyCode >= (int) Keys.D1 && keyCode <= (int) Keys.D9)
				{
					var tabIndex = keyCode - (int) Keys.D1;
					SwitchTabRequested?.Invoke(tabIndex);
				}

				if (ctrl && keyCode >= (int) Keys.NumPad1 && keyCode <= (int) Keys.NumPad9)
				{
					var tabIndex = keyCode - (int) Keys.NumPad1;
					SwitchTabRequested?.Invoke(tabIndex);
				}

...
namespace SafeExamBrowser.Browser
{
	internal class BrowserWindow : Contracts.IBrowserWindow
	{
		...
		internal event TabIndexRequestedEventHandler SwitchTabRequested;
		...
		internal void InitializeControl()
		{
			...
			keyboardHandler.NewTabRequested += KeyboardHandler_NewTabRequested;
			keyboardHandler.CloseTabRequested += KeyboardHandler_CloseTabRequested;
			keyboardHandler.SwitchTabRequested += KeyboardHandler_SwitchTabRequested;
		...
		private void KeyboardHandler_NewTabRequested()
		{
			logger.Info("User requested new tab via Ctrl+T.");
			NewTabRequested?.Invoke();
		}

		private void KeyboardHandler_CloseTabRequested()
		{
			if (IsMainWindow)
			{
			...
			{
				logger.Info("User requested to close tab via Ctrl+W.");
				CloseTabRequested?.Invoke();
			}
		}

		private void KeyboardHandler_SwitchTabRequested(int tabIndex)
		{
			logger.Info($"User requested to switch to tab {tabIndex} via Ctrl+{tabIndex + 1}.");
			SwitchTabRequested?.Invoke(tabIndex);
		}

...
namespace SafeExamBrowser.UserInterface.Desktop.Windows
{
	internal partial class SplashScreen : Window, ISplashScreen
	{
...
#if RELAXED_MODE
			// In relaxed mode, disable topmost and keep hidden
			Topmost = false;
			Visibility = Visibility.Hidden;
#else
			// Keep splash screen hidden from the start
			Visibility = Visibility.Hidden;
#endif
		}
		...
		{
			// Keep splash screen hidden - don't show it on screen
			Dispatcher.Invoke(() =>
			{
				// Do nothing - splash screen remains hidden
			});
		}
